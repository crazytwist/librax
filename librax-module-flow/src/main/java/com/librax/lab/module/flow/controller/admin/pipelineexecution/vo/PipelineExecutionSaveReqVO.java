...
@Schema(description = "管理后台 - 流程执行实例，支持完整流程、节点单独运行、补偿执行新增/修改 Request VO")
@Data
public class PipelineExecutionSaveReqVO {
    ...
    @NotEmpty(message = "PENDING | RUNNING | PAUSED | SUCCESS | FAILED | CANCELLED | COMPENSATING | COMPENSATED不能为空")
    private String status;

    @Schema(description = "MANUAL | SCHEDULE | EVENT | RETRY | STANDALONE", requiredMode = Schema.RequiredMode.REQUIRED, example = "2")
    @NotEmpty(message = "MANUAL | SCHEDULE | EVENT | RETRY | STANDALONE不能为空")
    ...
    @NotNull(message = "乐观锁，状态流转时 WHERE row_version=#{v} 防并发不能为空")
