...
@Schema(description = "管理后台 - 步骤定义表，可复用的步骤组件库 [pd_]新增/修改 Request VO")
@Data
public class StepDefinitionSaveReqVO {
    ...
    @NotEmpty(message = "步骤唯一标识，小写_下划线，如 ph_measure不能为空")
    ...
    @NotEmpty(message = "步骤显示名称，如 PH检测不能为空")
    private String name;
    ...
    @NotEmpty(message = "INSTRUMENT 仪器 | COMPUTE 计算 | CONDITION 条件 | WAIT 等待 | NOTIFY 通知不能为空")
    ...
    @NotNull(message = "是否支持单独运行 1 支持 0 不支持不能为空")
    ...
    @NotEmpty(message = "ACTIVE 启用  DISABLED 停用不能为空")
