...
@Schema(description = "管理后台 - 步骤执行记录，每次重试独立一行，支持单独运行与补偿追踪分页 Request VO")
@Data
public class StepExecutionPageReqVO extends PageParam {

    @Schema(description = "关联 pe_pipeline_execution.execution_id", example = "2067")
    private String executionId;

    @Schema(description = "节点 ID，对应 pd_pipeline_step.node_id", example = "6933")
    private String nodeId;

    @Schema(description = "冗余步骤标识，对应 pd_step_definition.step_key，便于按类型统计")
    private String stepKey;

    @Schema(description = "INSTRUMENT | COMPUTE | CONDITION | WAIT | NOTIFY", example = "1")
    private String stepType;

    @Schema(description = "第几次尝试，1-based，每次重试 INSERT 新行不覆盖历史")
    private Integer attempt;

    @Schema(description = "PENDING | RUNNING | SUCCESS | FAILED | SKIPPED | DEAD | COMPENSATING | COMPENSATED", example = "2")
    private String status;

    @Schema(description = "NORMAL | STANDALONE | COMPENSATE | MOCK")
