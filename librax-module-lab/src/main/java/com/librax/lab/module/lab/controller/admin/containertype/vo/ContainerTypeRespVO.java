...
@Schema(description = "管理后台 - 容器类型定义，描述各类实验容器的规格和层级结构，归 lab 模块管理 Response VO")
@Data
...
public class ContainerTypeRespVO {
    ...
    @ExcelProperty("容器类型唯一编码，如 PLATE_96_WELL / TUBE_EP_15ML / BOTTLE_50ML")
    private String typeCode;

    @Schema(description = "容器类型名称，如 96孔板 / 15ml EP管 / 50ml试剂瓶", requiredMode = Schema.RequiredMode.REQUIRED, example = "李四")
    @ExcelProperty("容器类型名称，如 96孔板 / 15ml EP管 / 50ml试剂瓶")
    ...
    @ExcelProperty("容器大类：PLATE=孔板 TUBE=试管/离心管 BOTTLE=瓶 RACK=托盘/架 VIAL=小瓶 TIP=吸头 CHIP=芯片 FILTER=滤膜 BOX=盒")
    ...
    @ExcelProperty("是否启用：1=启用 0=停用（停用后不能创建该类型实例）")
