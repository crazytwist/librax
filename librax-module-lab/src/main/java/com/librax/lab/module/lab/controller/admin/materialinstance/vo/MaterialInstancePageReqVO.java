...
@Schema(description = "管理后台 - 物料实例，库存中每一个具体的容器实例，含层级关系和位置追踪，归 lab 模块管理分页 Request VO")
@Data
public class MaterialInstancePageReqVO extends PageParam {

    @Schema(description = "实例唯一ID，UUID格式，如 INST-20260501-0001", example = "12596")
    private String instanceId;

    @Schema(description = "容器类型编码，关联 lab_container_type.type_code，如 PLATE_96_WELL / TUBE_EP_15ML")
    ...
    @Schema(description = "实例状态：AVAILABLE=可用 RESERVED=已预留（步骤申请但未取用） IN_USE=使用中 USED=已使用完 EXPIRED=已过期 DISCARDED=已废弃", example = "1")
