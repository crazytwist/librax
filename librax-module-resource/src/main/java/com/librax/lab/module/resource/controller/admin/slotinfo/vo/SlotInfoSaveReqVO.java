...
@Schema(description = "管理后台 - 库位定义，slot_id 同时作为资源ID纳入调度，归 resource 模块管理新增/修改 Request VO")
@Data
public class SlotInfoSaveReqVO {
    ...
    @NotEmpty(message = "库位唯一编码，同时作为 lab_resource_config.resource_id。固定库位如 RACK-A-01，AGV库位如 AGV-01-SLOT-1不能为空")
    ...
    @NotEmpty(message = "库位类型：FIXED=固定台面库位（有世界坐标） AGV=AGV载台槽位（无固定坐标） DEVICE=设备内部位置不能为空")
    ...
    @NotEmpty(message = "用途限制：ANY=不限 SAMPLE=样本 REAGENT=试剂 WASTE=废液 BUFFER=缓冲液 TIP=吸头不能为空")
    ...
    @NotNull(message = "最大容纳数量，一般为1（一个库位放一个容器），特殊场景可设大不能为空")
    private Integer capacity;

    @Schema(description = "是否启用：1=启用参与调度 0=禁用（维修/封存时禁用）", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotNull(message = "是否启用：1=启用参与调度 0=禁用（维修/封存时禁用）不能为空")
