...
依赖：docker compose v2，删除`name: librax-system`，降低`version`版本为`3.3`以下，支持`docker-compose`。
...
```shell
# 创建maven缓存volume
docker volume create --name librax-maven-repo

docker run -it --rm --name librax-maven \
    -v librax-maven-repo:/root/.m2 \
